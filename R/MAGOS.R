...
mag.step.check<-function(x.el){
  col1<- colSums(x.el*c(1:dim(x.el)[1]))
  return(col1)
}
mag.dist.v3<- function(vafs, depths){
  ...
  R <-c(dbeta(vafs[vafs!=0.001], shape1=s1, shape2=s2, log=T),
        2*dbeta(vafs[vafs==0.001], shape1=s1, shape2=s2, log=T)) #log should be F- because if Log=T we could have positive and negative values. We should add log
  #mean(R)
  #v <- sd(vafs)
  #v<-v*sqrt(length(vafs))
  #v<- var(vafs)
  ...
  #range<- sqrt(range)
  #range<- range^2
  #sk<- abs(kurtosis(vafs))
  #range<- quantile(vafs,0.90)-quantile(vafs,.1)
  #outies<- ifelse(length(vafs)>10 & length(boxplot.stats(vafs)$out)>3,log(length(boxplot.stats(vafs)$out),2) ,1)
...
#### AUGUST 2019: worsk on 1 sample_ laml
mag.exp.var.v3<-function(efrq.vec, edep.vec, num, n=500, type='M'){
  ...
  if(mean(edep.vec)>400){edep.vec<- edep.vec*400/mean(edep.vec)}
  a<-quantile(edep.vec,0.25)
  b<-quantile(edep.vec,0.75)



  lowerf.1 <- quantile(efrq.vec,0.05)
  higherf.1 <- quantile(efrq.vec,0.95);
  #higherf.1 <- max(efrq.vec);
  #lowerf.1 <- min(efrq.vec);

  #cat(min(efrq.vec), max(efrq.vec), ',', lowerf, higherf, ',', lowerf.1, higherf.1, '\n');
  #  boxplot(efrq.vec);
  #  readline()
  if(length(edep.vec)>10){
    edep.vec<- edep.vec[edep.vec>=a&
                          edep.vec<=b]
  } else {
    edep.vec<-c(mean(edep.vec), mean(edep.vec))
  }
  if(length(efrq.vec)>10){
  ...
  for(i in 1:n){
    ...
    #efrq<- mean(efrq.vec)     # i dont want to sample from frq. bc it will mess up the freq dist of the cluster
    edep<- mean(edep.vec)
    s1<-efrq*edep
    s2<-(1-efrq)*edep
    ...
    for(jj in 1:num){
      ...
      mut<- rbeta(1, shape1=efrq*edep,shape2= (1-efrq)*edep)
      muts<- c(muts, mut)
    }
    expt.var.mut<- c(expt.var.mut, var(muts))
  }
  return(list(exp.vat=(expt.var), depths=depth.keep,
              vafs=vafs,exp.mut=expt.var.mut, muts=muts))
}
...
mag.single.v3.1 <- function(prep.data){
...
cut.off.single.v3.1 <- function(mag.output){
  time0 <- as.numeric(Sys.time())
...
#'
#' @param arg.data Preproccessed dataframe of the data.
...
#'
#' @export
mag.reduce.graph.v3.1<-function(prep.data,x.el=matrix(), n=-100, threshold=0.03){
  ...
  if(nn < size.threshold | nn == n) {
    ...
    mag.reduce.graph.v3.1(prep.data = prep.data,x.el=x.el.final,n=nn, threshold=threshold)
  }
...
#' @export
mag.multiple.v3.1<- function(prep.data,x.el.cut=data.frame()) {
...
cut.off.multiple.v3.2  <-function(mag.var){
  time0 <- as.numeric(Sys.time())
  #var.mean <- mag.output$var.mean
    ...
    for( j in 1:length(mag.var.list)){
      ...
      if(sum(ok.points & stepI.x.el.step) == 0){     ###meaning stepI.x.el is not in ok.points
        ...
        for(ii in 1:2){
          ...
          if(length(vaf.vec)>5 & max(vaf.vec)-min(vaf.vec)>0.05){
            print('UMAD TOOOOOOOOO')
            ### added the second condition to ignore the clusters with all equal vafs.
          ...
          print('NNNANNNANANANAN')
          print(var(vaf.vec))
          print(vaf.vec)
          if(stepII.breaks[ii, 'okay'] >0 |
             #floor(var.check*10^4)/10^4 <  ceiling((round(m+3*vv,4)+0.00001)*10^4)/10^4) {
             floor(var.check*10^4)/10^4< m+3*vv | var(vaf.vec)<0.00001){
            #test$p.value>0.05){
            #print('okay shod:')
            #print(test)
            keep[ii]<-keep[ii]+1
...
mag.var.v3.1<- function(mag.out){
  ...
  ### feb 28
  x.el.red.IDs<- as.numeric(colnames(x.el.list[[1]]))
...
#'
#' @export
mag.run<- function(data,name="",sampleFreqs= NA,sampleNum=1, shape=10, depth=200){
  if(length(data$ID)==0){return(print("RUN mag.prepdata"))}
  if(sum(is.na(sampleFreqs))>0){ data=data }else{data=cbind(data[,sampleFreqs], ID=data$ID)}
  #data.prep<- mag.prepdata(data)
  # data should have id.
  depth<<- depth
  data.prep<- data
  dim<-dim(data)[2]
  ...
  if(dim==2){
    ...
    mag.out.single<- mag.single(data.prep, sampleNum=s)
    mag.cut.single<- find.cut.off(mag.out.single, shape=shape, depth=depth)
    results<- list(final.dataframe<- mag.cut.single$final.data, mag.output<- mag.out.single,
                   data.prep<- data.prep)
    names(results)<- c(paste(name,".final.data", sep = ""),paste(name,".mag.single.output", sep = ""),
  ...
  if(dim>2){
    ...
    mag.red<-c()
    if(dim(data)[1]>10){
    mag.red<- mag.reduce.graph(data.prep)
    mag.out<- mag.multiple.cut(data.prep, x.el.cut = mag.red$x.el.red, sampleNum=s)
    }
    mag.out<- mag.multiple.cut(data.prep, sampleNum=s)
    mag.mv<- mag.var(mag.out,sampleNum = s)
    mag.cut<- find.cut.off.multiple(mag.mv, shape=shape, depth=depth)
    results<- list(final.dataframe<-mag.cut$final.data.clusters ,data.prep<- data.prep,
                   reduce.output<- mag.red, mag.output<- mag.out, mag.cutoff<- mag.cut, mag.mvar= mag.mv )
    names(results)<- c(paste(name,".final.data", sep = ""), paste(name,".data.prep",sep = ""),
                       paste(name,".reduce.output", sep = ""),
                       paste(name,".mag.out", sep = ""), paste(name,".mag.cut", sep = ""),
                       paste(name,".mag.vars", sep=""))
