...
mag.reduce.graph<-function(prep.data,x.el=matrix(), n=-100){
  ...
  if(nn < size.threshold | nn == n) {
    ...
    mag.reduce.graph(prep.data = prep.data,x.el=x.el.final,n=nn)
  }
...
mag.multiple.run<- function(input.data, reduce=T){
