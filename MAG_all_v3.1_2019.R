######### MARCH 2019: FINAL VERSION- working great on cell plat 
...
##### TEMP CHANGES FEB 15 2019----- FEB17: works on cell low coverage
...
##### TEMP CHANGES FEB 15 2019----- MARCH--- lower coverage--- works okay on low, med, high
mag.dist.v3<- function(vafs, depths){
  ...
  R <-c(dbeta(vafs[vafs!=0.001], shape1=s1, shape2=s2, log=T),2*dbeta(vafs[vafs==0.001], shape1=s1, shape2=s2, log=T)) #log should be F- because if Log=T we could have positive and negative values. We should add log
#mean(R)
  #v <- sd(vafs)
  #v<-v*sqrt(length(vafs))
  #v<- var(vafs)
...
fit.elements.v3<- function(elements,vaf.data, depth.data){
  ...
  #return(c(max(fit.obj),fit.minval[which.max(fit.obj)]))
  #return(c(max(dist.val),dist.shape[which.max(dist.val)]))
  #result=result[result[,'dist']<10,,drop=F]
  return(result[which.max(result[,'dist']), c('dist', 's1')])
}
...
mag.exp.var.v3<-function(efrq.vec, edep.vec, num, n=500, type='M'){
  ...
  #lower<-a-1.5*(b-a)
  #higher<- b+1.5*(b-a)
  #lower.1 <- quantile(edep.vec,0.005)
  #higher.1 <- quantile(edep.vec,0.995);
  #cat(min(edep.vec), max(edep.vec), ',', lower, higher, ',', lower.1, higher.1, '\n');
  #boxplot(edep.vec); 
  #readline();
  #af<-quantile(efrq.vec,0.25)
  #bf<-quantile(efrq.vec,0.75)
  #lowerf<-af-1.5*(bf-af)
  #higherf<- bf+1.5*(bf-af)
  lowerf.1 <- quantile(efrq.vec,0.25)
  higherf.1 <- quantile(efrq.vec,0.75);
...
mag.exp.var.v3<-function(efrq.vec, edep.vec, num, n=500, type='M'){
  ...
  #depth.range<- max(edep.vec)-min(edep.vec)
  #if(sd(edep.vec)<10){edep.vec<- mean(edep.vec)}
  if(mean(edep.vec)>500){edep.vec<- edep.vec*500/mean(edep.vec)}
  a<-quantile(edep.vec,0.05)
  b<-quantile(edep.vec,0.95)
  #lower<-a-1.5*(b-a)
  #higher<- b+1.5*(b-a)
...
#### CHANGE x.el from dataframe to matrix
...
### in order to fix the var problem, (small var of the cluster with a lot of points on 0)
...
####add all depths: 
cut.off.multiple.v3.1  <-function(mag.var){
  time0 <- as.numeric(Sys.time())
